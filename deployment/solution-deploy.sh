...
usage() {
    ...
    echo "      --update-frontend Build and deploy frontend to existing stack (preserves aws-exports.json)"
...
# ===== FRONTEND UPDATE =====
# Build frontend and deploy to existing CloudFormation stack's S3 portal bucket
# Preserves aws-exports.json (generated by CloudFormation)

if [ "$UPDATE_FRONTEND_ONLY" = true ]; then
    STACK_NAME="cloudfront-s3-control-plane-stack-global"
    
    echo "=== Updating frontend for stack: ${STACK_NAME} ==="
    
    # Verify stack exists
    STACK_STATUS=$(aws cloudformation describe-stacks --stack-name ${STACK_NAME} \
        --region ${AWS_REGION} --profile ${AWS_DEFAULT_PROFILE} \
        --query 'Stacks[0].StackStatus' --output text 2>/dev/null)
    
    if [ -z "$STACK_STATUS" ] || [ "$STACK_STATUS" = "None" ]; then
        echo "Error: Stack ${STACK_NAME} not found. Deploy the control plane first."
        exit 1
    fi
    echo "Stack status: ${STACK_STATUS}"
    
    # Get portal bucket name
    PORTAL_BUCKET=$(aws cloudformation describe-stack-resources \
        --stack-name ${STACK_NAME} \
        --region ${AWS_REGION} --profile ${AWS_DEFAULT_PROFILE} \
        --query 'StackResources[?LogicalResourceId==`cloudfrontcontrolplaneportalbucket1534502E`].PhysicalResourceId' --output text)
    
    if [ -z "$PORTAL_BUCKET" ] || [ "$PORTAL_BUCKET" = "None" ]; then
        echo "Error: Portal bucket not found in stack resources."
        exit 1
    fi
    echo "Portal bucket: ${PORTAL_BUCKET}"
    
    # Backup aws-exports.json before sync
    echo "Backing up aws-exports.json..."
    aws s3 cp s3://${PORTAL_BUCKET}/aws-exports.json /tmp/aws-exports-backup.json \
        --region ${AWS_REGION} --profile ${AWS_DEFAULT_PROFILE}
    ...
    cd ..
    pnpm install
    pnpm nx build @aws/clickstream-base-lib
    cd frontend
    pnpm run build
    cd ../deployment
    
    # Upload new frontend (delete old files)
    echo "Uploading frontend to S3..."
    aws s3 sync ../frontend/build/ s3://${PORTAL_BUCKET}/ --delete \
        --region ${AWS_REGION} --profile ${AWS_DEFAULT_PROFILE}
    
    # Restore aws-exports.json
    echo "Restoring aws-exports.json..."
    aws s3 cp /tmp/aws-exports-backup.json s3://${PORTAL_BUCKET}/aws-exports.json \
        --region ${AWS_REGION} --profile ${AWS_DEFAULT_PROFILE}
    
    # Invalidate CloudFront cache
    CF_DIST=$(aws cloudformation describe-stack-resources \
        --stack-name ${STACK_NAME} \
        --region ${AWS_REGION} --profile ${AWS_DEFAULT_PROFILE} \
        --query 'StackResources[?ResourceType==`AWS::CloudFront::Distribution`].PhysicalResourceId' --output text)
    
    echo "Invalidating CloudFront cache (${CF_DIST})..."
    aws cloudfront create-invalidation --distribution-id ${CF_DIST} --paths "/*" \
        --region ${AWS_REGION} --profile ${AWS_DEFAULT_PROFILE} > /dev/null
    
    CF_DOMAIN=$(aws cloudfront get-distribution --id ${CF_DIST} \
        --region ${AWS_REGION} --profile ${AWS_DEFAULT_PROFILE} \
