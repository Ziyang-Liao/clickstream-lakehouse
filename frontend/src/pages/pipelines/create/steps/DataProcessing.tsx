...
interface DataProcessingProps {
  ...
  // S3 Tables modeling props
  changeEnableS3TablesModeling: (enable: boolean) => void;
  changeS3TableBucket: (bucket: SelectProps.Option) => void;
  changeS3TablesNamespace: (namespace: string) => void;
  changeS3TablesScheduleExpression: (expression: string) => void;
  changeS3TablesDataRetentionDays: (days: string) => void;
  ...
  s3TableBucketEmptyError: boolean;
  s3TableBucketArnFormatError: boolean;
  s3TablesNamespaceEmptyError: boolean;
  s3TablesScheduleFormatError: boolean;
  s3TablesDataRetentionError: boolean;
}
...
const DataProcessing: React.FC<DataProcessingProps> = (
  ...
  const {
    ...
    changeEnableS3TablesModeling,
    changeS3TableBucket,
    changeS3TablesNamespace,
    changeS3TablesScheduleExpression,
    changeS3TablesDataRetentionDays,
    ...
    // S3 Tables validation errors
    s3TableBucketEmptyError,
    s3TableBucketArnFormatError,
    s3TablesNamespaceEmptyError,
    s3TablesScheduleFormatError,
    s3TablesDataRetentionError,
  } = props;
            ...
            <SpaceBetween direction="vertical" size="s">
              <SpaceBetween direction="horizontal" size="xs">
                <Checkbox
                  ...
                  disabled={isDisabled(update, pipelineInfo) || pipelineInfo.enableS3TablesModeling}
              ...
              {/* S3 Tables Modeling Option */}
              <SpaceBetween direction="horizontal" size="xs">
                <Checkbox
                  controlId="test-s3tables-id"
                  disabled={isDisabled(update, pipelineInfo) || pipelineInfo.enableRedshift}
                  checked={pipelineInfo.enableS3TablesModeling}
                  onChange={(e) => {
                    changeEnableS3TablesModeling(e.detail.checked);
                  }}
                />
                <FormField
                  label={t('pipeline:create.s3Tables')}
                  description={t('pipeline:create.s3TablesDesc')}
                ></FormField>
              </SpaceBetween>

              {pipelineInfo.enableS3TablesModeling && (
                <SpaceBetween direction="vertical" size="l">
                  <FormField
                    label={t('pipeline:create.s3TableBucket')}
                    description={t('pipeline:create.s3TableBucketDesc')}
                    errorText={defaultTo(
                      ternary(
                        s3TableBucketEmptyError,
                        t('pipeline:valid.s3TableBucketEmptyError'),
                        undefined
                      ),
                      ternary(
                        s3TableBucketArnFormatError,
                        t('pipeline:valid.s3TableBucketArnFormatError'),
                        undefined
                      )
                    )}
                  >
                    <Input
                      disabled={isDisabled(update, pipelineInfo)}
                      placeholder={defaultStr(
                        t('pipeline:create.s3TableBucketPlaceholder')
                      )}
                      value={pipelineInfo.selectedS3TableBucket?.value || ''}
                      onChange={(e) => {
                        if (
                          new RegExp(XSS_PATTERN).test(e.detail.value) ||
                          e.detail.value.length > MAX_USER_INPUT_LENGTH
                  ...
                  <FormField
                    label={t('pipeline:create.s3TablesNamespace')}
                    description={t('pipeline:create.s3TablesNamespaceDesc')}
                    errorText={ternary(
                      s3TablesNamespaceEmptyError,
                      t('pipeline:valid.s3TablesNamespaceEmptyError'),
                      undefined
                    )}
                  >
                    <Input
                      disabled={isDisabled(update, pipelineInfo)}
                      placeholder={defaultStr(
                        t('pipeline:create.s3TablesNamespacePlaceholder')
                      )}
                      value={pipelineInfo.s3TablesNamespace}
                      onChange={(e) => {
                        if (
                          new RegExp(XSS_PATTERN).test(e.detail.value) ||
                          e.detail.value.length > MAX_USER_INPUT_LENGTH
                        ...
                        changeS3TablesNamespace(e.detail.value);
                      }}
                    />
                  </FormField>

                  <FormField
                    label={t('pipeline:create.s3TablesSchedule')}
                    description={t('pipeline:create.s3TablesScheduleDesc')}
                    errorText={ternary(
                      s3TablesScheduleFormatError,
                      t('pipeline:valid.s3TablesScheduleFormatError'),
                      undefined
                    )}
                  >
                    <Input
                      disabled={isDisabled(update, pipelineInfo)}
                      placeholder={defaultStr(
                        t('pipeline:create.s3TablesSchedulePlaceholder')
                      )}
                      value={pipelineInfo.s3TablesScheduleExpression}
                      onChange={(e) => {
                        if (
                          new RegExp(XSS_PATTERN).test(e.detail.value) ||
                          e.detail.value.length > MAX_USER_INPUT_LENGTH
                        ...
                        changeS3TablesScheduleExpression(e.detail.value);
                      }}
                    />
                  </FormField>

                  <FormField
                    label={t('pipeline:create.s3TablesDataRetention')}
                    description={t('pipeline:create.s3TablesDataRetentionDesc')}
                    errorText={ternary(
                      s3TablesDataRetentionError,
                      t('pipeline:valid.s3TablesDataRetentionError'),
                      undefined
                    )}
                  >
                    <Input
                      ...
                      disabled={isDisabled(update, pipelineInfo)}
                      placeholder={defaultStr(
                        t('pipeline:create.s3TablesDataRetentionPlaceholder')
                      )}
                      value={String(pipelineInfo.s3TablesDataRetentionDays)}
                      onChange={(e) => {
                        if (!POSITIVE_INTEGER_REGEX.test(e.detail.value)) {
                          return false;
                        }
                        changeS3TablesDataRetentionDays(e.detail.value);
                      }}
                    />
                  </FormField>
                </SpaceBetween>
