...
const Content: React.FC<ContentProps> = (props: ContentProps) => {
  ...
  // S3 Tables validation error states
  const [s3TableBucketEmptyError, setS3TableBucketEmptyError] = useState(false);
  const [s3TableBucketArnFormatError, setS3TableBucketArnFormatError] = useState(false);
  const [s3TablesNamespaceEmptyError, setS3TablesNamespaceEmptyError] =
    useState(false);
  const [s3TablesScheduleFormatError, setS3TablesScheduleFormatError] =
    useState(false);
  const [s3TablesDataRetentionError, setS3TablesDataRetentionError] =
    useState(false);
  ...
  const checkS3TablesConfig = (info: IExtPipeline) => {
    // Validate S3 Table Bucket ARN is not empty
    if (!info.selectedS3TableBucket?.value) {
      setS3TableBucketEmptyError(true);
      return false;
    }

    // Validate S3 Table Bucket ARN format
    // Format: arn:aws(-cn|-us-gov)?:s3tables:{region}:{account}:bucket/{bucket-name}
    const s3TableBucketArnPattern = /^arn:aws(-cn|-us-gov)?:s3tables:[a-z0-9-]+:[0-9]{12}:bucket\/[a-z0-9.-]+$/;
    if (!s3TableBucketArnPattern.test(info.selectedS3TableBucket.value)) {
      setS3TableBucketArnFormatError(true);
      return false;
    }

    // Validate namespace is not empty and has valid format
    const namespace = info.s3TablesNamespace?.trim();
    if (!namespace) {
      setS3TablesNamespaceEmptyError(true);
      return false;
    }
    
    // Namespace should be alphanumeric with underscores, starting with a letter
    ...
    const scheduleExp = info.s3TablesScheduleExpression?.trim();
    if (!scheduleExp) {
      setS3TablesScheduleFormatError(true);
      return false;
    }
    
    // Check if it starts with 'cron(' and ends with ')'
    const cronMatch = scheduleExp.match(/^cron\((.+)\)$/);
    if (!cronMatch) {
      setS3TablesScheduleFormatError(true);
      return false;
    }
    
    // Check if cron expression has 6 fields
    ...
    // Validate data retention days (1-3650)
    const retentionDays = info.s3TablesDataRetentionDays;
    if (
      !retentionDays ||
      retentionDays < 1 ||
      retentionDays > 3650
    ) {
      setS3TablesDataRetentionError(true);
      return false;
    }
  ...
  const validateDataProcessing = () => {
    ...
    // Validate S3 Tables configuration when enabled
    if (pipelineInfo.enableS3TablesModeling) {
      if (!checkS3TablesConfig(pipelineInfo)) {
        return false;
      }
    }
          ...
          content: (
            <DataProcessing
              ...
              changeEnableRedshift={(enable) => {
                if (enable) {
                  // if enable redshift, default to enable athena and quicksight, disable S3 Tables
                  setPipelineInfo((prev) => {
                    return {
                      ...prev,
                      enableRedshift: true,
                      enableS3TablesModeling: false,
                      // Enable QuickSight When QuickSight Available
                      enableReporting: ternary(
                        prev.serviceStatus.QUICK_SIGHT,
                        true,
              ...
              // S3 Tables modeling handlers
              changeEnableS3TablesModeling={(enable) => {
                if (enable) {
                  // When enabling S3 Tables, disable Redshift
                  setPipelineInfo((prev) => {
                    return {
                      ...prev,
                      enableS3TablesModeling: true,
                      enableRedshift: false,
                      enableReporting: false,
              ...
              changeS3TableBucket={(bucket) => {
                setS3TableBucketEmptyError(false);
                setS3TableBucketArnFormatError(false);
                setPipelineInfo((prev) => {
                  return {
                    ...prev,
                    selectedS3TableBucket: bucket,
                    dataModeling: {
                      ...prev.dataModeling,
                      s3Tables: {
                        ...prev.dataModeling.s3Tables,
                        tableBucketArn: defaultStr(bucket.value),
                      },
                    },
                  };
                });
              }}
              changeS3TablesNamespace={(namespace) => {
                setS3TablesNamespaceEmptyError(false);
                setPipelineInfo((prev) => {
                  return {
                    ...prev,
                    s3TablesNamespace: namespace,
                    dataModeling: {
                      ...prev.dataModeling,
                      s3Tables: {
                        ...prev.dataModeling.s3Tables,
              ...
              changeS3TablesScheduleExpression={(expression) => {
                setS3TablesScheduleFormatError(false);
                setPipelineInfo((prev) => {
                  return {
                    ...prev,
                    s3TablesScheduleExpression: expression,
                    dataModeling: {
                      ...prev.dataModeling,
                      s3Tables: {
                        ...prev.dataModeling.s3Tables,
              ...
              changeS3TablesDataRetentionDays={(days) => {
                setS3TablesDataRetentionError(false);
                setPipelineInfo((prev) => {
                  return {
                    ...prev,
                    s3TablesDataRetentionDays: parseInt(days) || 365,
                    dataModeling: {
                      ...prev.dataModeling,
                      s3Tables: {
                        ...prev.dataModeling.s3Tables,
                        dataRetentionDays: parseInt(days) || 365,
                      },
                    },
                  };
                });
              }}
              s3TableBucketEmptyError={s3TableBucketEmptyError}
              s3TableBucketArnFormatError={s3TableBucketArnFormatError}
              s3TablesNamespaceEmptyError={s3TablesNamespaceEmptyError}
              s3TablesScheduleFormatError={s3TablesScheduleFormatError}
              s3TablesDataRetentionError={s3TablesDataRetentionError}
            />
          ),
        },
        {
