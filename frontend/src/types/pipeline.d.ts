...
declare global {
  ...
  /**
   * S3 Tables data modeling configuration
   * Validates: Requirements 9.1, 9.2
   */
  interface IS3TablesModelingConfig {
    readonly tableBucketArn?: string;      // S3 Table Bucket ARN
    readonly namespace?: string;           // Namespace, default: clickstream_{projectId}
    readonly scheduleExpression?: string;   // Schedule expression (e.g., 'cron(0 2 * * ? *)')
  ...
  interface IPipeline {
    ...
    dataModeling: {
      ...
      s3Tables?: IS3TablesModelingConfig;
    };
    reporting: {
      quickSight: {
        accountName: string;
  ...
  interface IExtPipeline extends IPipeline {
    ...
    // S3 Tables data modeling temporary properties
    // Validates: Requirements 9.3
    enableS3TablesModeling: boolean;
    selectedS3TableBucket: SelectProps.Option | null;
    s3TablesNamespace: string;
    s3TablesScheduleExpression: string;
    s3TablesDataRetentionDays: number;
  }
