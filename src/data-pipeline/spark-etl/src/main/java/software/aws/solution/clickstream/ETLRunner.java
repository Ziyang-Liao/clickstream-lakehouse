...
public class ETLRunner {
    ...
    public static final String FIELD_FILTER_FILE_NAME = "field_filter_rule.json";
    ...
    public ETLRunner(final SparkSession spark, final ETLRunnerConfig runConfig) {
        ...
        this.fieldFilterTransformer = new FieldFilterTransformer();
        this.fieldFilterConfigLoader = new FieldFilterConfigLoader();
    ...
    void initConfig(final ETLRunnerConfig runConfig) {
        ...
        Map<String, FieldFilterRuleConfig> appFieldFilterConfig = new HashMap<>();
        ...
        for (PathContent pathContent : configFileList) {
            ...
            // Load field filter rules
            if (FIELD_FILTER_FILE_NAME.equalsIgnoreCase(fileName)) {
                FieldFilterRuleConfig fieldFilterRule = parseFieldFilterRule(content, appId);
                if (fieldFilterRule != null) {
                    appFieldFilterConfig.put(appId, fieldFilterRule);
                    log.info("Loaded field filter rule for appId: " + appId);
                }
            }
        }

        // Also load field filter rules using the dedicated loader
        fieldFilterConfigLoader.loadRules(this.spark, ruleConfigDir);
        ...
        transformRuleConfig.setAppFieldFilterConfig(appFieldFilterConfig);
    ...
    private FieldFilterRuleConfig parseFieldFilterRule(final String content, final String appId) {
        try {
            com.fasterxml.jackson.databind.ObjectMapper mapper = new com.fasterxml.jackson.databind.ObjectMapper();
            Map<String, Object> jsonMap = mapper.readValue(content,
                    new com.fasterxml.jackson.core.type.TypeReference<Map<String, Object>>() { });

            FieldFilterRuleConfig.FieldFilterRuleConfigBuilder builder = FieldFilterRuleConfig.builder();
            ...
            if (jsonMap.containsKey("filterMode")) {
                builder.filterMode((String) jsonMap.get("filterMode"));
            }
            ...
            log.error("Failed to parse field filter rule JSON: {}", e.getMessage());
            return null;
        }
    }
    ...
    public Dataset<Row> executeTransformers(final Dataset<Row> dataset,
        ...
        Dataset<Row> postTransformResult = execPostTransform(result, transformerClassNames.get(0));

        // Apply field filtering after all transformations
        postTransformResult = applyFieldFiltering(postTransformResult);

        return postTransformResult;
    }

    /**
     * Apply field filtering to the dataset based on configured filter rules.
     * When multiple apps have different rules, the dataset is partitioned by app_id
     * and each partition is filtered independently, then unioned back together.
     *
     * @param dataset The dataset to filter
     * @return The filtered dataset
     */
    private Dataset<Row> applyFieldFiltering(final Dataset<Row> dataset) {
        Map<String, FieldFilterRuleConfig> appFieldFilterConfig = this.transformConfig.getAppFieldFilterConfig();
        if (appFieldFilterConfig == null || appFieldFilterConfig.isEmpty()) {
            log.info("No field filter rules configured, skipping field filtering");
            return dataset;
        }

        log.info("Applying field filtering with {} configured rules", appFieldFilterConfig.size());

        String pipelineId = null;
        // Extract pipelineId from loaded pipeline rules (since ETLRunnerConfig doesn't carry it)
        Map<String, FieldFilterRule> pipelineRules = fieldFilterConfigLoader.getPipelineRules();
        if (!pipelineRules.isEmpty()) {
            pipelineId = pipelineRules.keySet().iterator().next();
        }
        String[] validAppIds = runConfig.getValidAppIds().split(",");

        // Collect effective rules per app
        Map<String, FieldFilterRule> effectiveRules = new HashMap<>();
        for (String appId : validAppIds) {
            String trimmedAppId = appId.trim();
            if (trimmedAppId.isEmpty()) {
                continue;
            }
            // Try config loader first (from S3 JSON files)
            Optional<FieldFilterRule> rule = fieldFilterConfigLoader.getEffectiveRule(trimmedAppId, pipelineId);
            if (rule.isPresent()) {
                effectiveRules.put(trimmedAppId, rule.get());
            } else {
                // Fall back to inline config
                FieldFilterRuleConfig inlineConfig = appFieldFilterConfig.get(trimmedAppId);
                if (inlineConfig != null && inlineConfig.getFilterMode() != null) {
                    effectiveRules.put(trimmedAppId, convertToFieldFilterRule(inlineConfig));
                }
            }
        }

        if (effectiveRules.isEmpty()) {
            log.info("No applicable field filter rules found, returning original dataset");
            return dataset;
        }

        // Check if all rules are identical — if so, apply once to the whole dataset
        FieldFilterRule firstRule = effectiveRules.values().iterator().next();
        boolean allSame = effectiveRules.values().stream().allMatch(r ->
                r.getFilterMode() == firstRule.getFilterMode()
                && r.getFields().equals(firstRule.getFields()));

        if (allSame) {
            log.info("All apps share the same filter rule, applying once to entire dataset");
            return fieldFilterTransformer.transform(dataset, firstRule);
        }

        // Different rules per app — partition, filter, and union
        log.info("Different filter rules per app, applying per-app filtering");
        Dataset<Row> result = null;
        Set<String> processedApps = new HashSet<>();

        for (Map.Entry<String, FieldFilterRule> entry : effectiveRules.entrySet()) {
            String appId = entry.getKey();
            FieldFilterRule rule = entry.getValue();
            Dataset<Row> appData = dataset.filter(col(Constant.APP_ID).equalTo(appId));
            Dataset<Row> filtered = fieldFilterTransformer.transform(appData, rule);
            result = (result == null) ? filtered : result.unionByName(filtered, true);
            processedApps.add(appId);
        }

        // Include data from apps without filter rules (unfiltered)
        Dataset<Row> unprocessedData = dataset;
        for (String appId : processedApps) {
            unprocessedData = unprocessedData.filter(col(Constant.APP_ID).notEqual(appId));
        }
        if (!unprocessedData.isEmpty()) {
            result = (result == null) ? unprocessedData : result.unionByName(unprocessedData, true);
        }
    ...
    /**
     * Convert FieldFilterRuleConfig to FieldFilterRule.
     */
    private FieldFilterRule convertToFieldFilterRule(final FieldFilterRuleConfig config) {
        FieldFilterRule.FilterMode filterMode = null;
        if ("whitelist".equalsIgnoreCase(config.getFilterMode())) {
            filterMode = FieldFilterRule.FilterMode.WHITELIST;
        } else if ("blacklist".equalsIgnoreCase(config.getFilterMode())) {
            filterMode = FieldFilterRule.FilterMode.BLACKLIST;
        }

        return FieldFilterRule.builder()
