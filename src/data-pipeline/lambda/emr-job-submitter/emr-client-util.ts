...
export class EMRServerlessUtil {
  ...
  private static async getJobRunCommandInput(event: any, config: any, startTimestamp: number, endTimestamp: number,
    ...
    const entryPointArguments = [
      ...
      config.filterBotEvent, // [18] filterBotEvent
    ];
...
async function putInitRuleConfig(ruleConfigDir: string, appIds: string) {
  ...
  for (const appId of appIds.split(',')) {
    ...
    // Always put traffic source rules (they use default if not customized)
  ...
  // Sync field filter rules from DynamoDB to S3
  await syncFieldFilterRulesToS3(bucket, keyPefix, appIds);
}

/**
 * Sync field filter rules from DynamoDB to S3.
 * This ensures the Spark ETL job can read the latest filter rules.
 */
async function syncFieldFilterRulesToS3(bucket: string, keyPrefix: string, appIds: string) {
  ...
  if (!tableName || !projectId) {
    logger.warn('CLICK_STREAM_TABLE_NAME or PROJECT_ID not set, skipping field filter rule sync');
    return;
  }

  if (!pipelineId) {
    logger.warn('PIPELINE_ID not set, skipping field filter rule sync. Ensure the Data Processing stack includes the PipelineId parameter.');
    return;
  }

  logger.info('syncFieldFilterRulesToS3', { bucket, keyPrefix, appIds, tableName, projectId, pipelineId });

  try {
    // Get pipeline-level filter rule
    const pipelineRule = await getFieldFilterRuleFromDynamoDB(tableName, projectId!, pipelineId!, undefined);

    // Process each app
    for (const appId of appIds.split(',')) {
      const trimmedAppId = appId.trim();
      if (!trimmedAppId) continue;

      // Get app-level filter rule
      const appRule = await getFieldFilterRuleFromDynamoDB(tableName, projectId!, pipelineId!, trimmedAppId);

      // Use app rule if exists, otherwise use pipeline rule
      const effectiveRule = appRule || pipelineRule;

      if (effectiveRule) {
        const fieldFilterRuleKey = `${keyPrefix}${trimmedAppId}/${FIELD_FILTER_RULE_FILE_NAME}`;
        const ruleContent = JSON.stringify({
          projectId: effectiveRule.projectId,
          pipelineId: effectiveRule.pipelineId,
          appId: trimmedAppId,
          filterMode: effectiveRule.filterMode,
          fields: effectiveRule.fields,
        });

        await putStringToS3(ruleContent, bucket, fieldFilterRuleKey);
        logger.info(`Synced field filter rule to s3://${bucket}/${fieldFilterRuleKey}`, {
          source: appRule ? 'app' : 'pipeline',
          filterMode: effectiveRule.filterMode,
          fieldCount: effectiveRule.fields?.length || 0,
        });
      } else {
        logger.info(`No field filter rule found for appId: ${trimmedAppId}`);
      }
    }
  } catch (error) {
    logger.error('Failed to sync field filter rules to S3', error as Error);
    // Don't throw - allow ETL to continue without field filtering
  }
}

/**
 * Get field filter rule from DynamoDB.
 */
async function getFieldFilterRuleFromDynamoDB(
  ...
  try {
    const typeValue = appId
      ? `FILTER_RULE#app#${appId}`
      : `FILTER_RULE#pipeline#${pipelineId}`;
    ...
    if (response.Items && response.Items.length > 0) {
      ...
      // Check if the rule is not deleted
    ...
    logger.error(`Failed to get field filter rule from DynamoDB: ${error}`);
    return undefined;
  }
}

interface FieldFilterRuleRecord {
  projectId: string;
  pipelineId: string;
  appId?: string;
  filterMode: string;
