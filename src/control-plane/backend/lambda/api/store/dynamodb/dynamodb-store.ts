...
export class DynamoDbStore implements ClickStreamStore {
  ...
  /**
   * Add a new field filter rule
   * @param rule The field filter rule to add
   * @returns The rule type (used as identifier)
   */
  public async addFieldFilterRule(rule: IFieldFilterRule): Promise<string> {
    const ruleType = rule.appId
      ? `FILTER_RULE#app#${rule.appId}`
      : `FILTER_RULE#pipeline#${rule.pipelineId}`;
  ...
  /**
   * Get a field filter rule
   * @param projectId The project ID
   * @param pipelineId The pipeline ID
   * @param appId Optional application ID (if provided, gets app-level rule; otherwise gets pipeline-level rule)
   * @returns The field filter rule or undefined if not found
   */
  public async getFieldFilterRule(projectId: string, pipelineId: string, appId?: string): Promise<IFieldFilterRule | undefined> {
    const ruleType = appId
    ...
    const rule: IFieldFilterRule = result.Item as IFieldFilterRule;
    return !rule.deleted ? rule : undefined;
  };

  /**
   * Update a field filter rule
   * @param rule The field filter rule to update
   */
  public async updateFieldFilterRule(rule: IFieldFilterRule): Promise<void> {
    const ruleType = rule.appId
      ? `FILTER_RULE#app#${rule.appId}`
      : `FILTER_RULE#pipeline#${rule.pipelineId}`;
    ...
    const params: UpdateCommand = new UpdateCommand({
      ...
      UpdateExpression: 'SET #updateAt = :u, #operator = :operator, #filterMode = :filterMode, #fields = :fields',
      ExpressionAttributeNames: {
        '#updateAt': 'updateAt',
        '#operator': 'operator',
  ...
  /**
   * Delete a field filter rule (soft delete)
   * @param projectId The project ID
   * @param pipelineId The pipeline ID
   * @param appId Optional application ID
   * @param operator The operator performing the deletion
   */
  public async deleteFieldFilterRule(projectId: string, pipelineId: string, appId: string | undefined, operator: string): Promise<void> {
    const ruleType = appId
      ? `FILTER_RULE#app#${appId}`
      : `FILTER_RULE#pipeline#${pipelineId}`;
    ...
    const params: UpdateCommand = new UpdateCommand({
      ...
      UpdateExpression: 'SET deleted = :d, #operator = :operator, #updateAt = :u',
      ExpressionAttributeNames: {
        '#operator': 'operator',
        '#updateAt': 'updateAt',
      },
  ...
  /**
   * List all field filter rules for a pipeline (including app-level rules)
   * @param projectId The project ID
   * @param pipelineId The pipeline ID
   * @returns Array of field filter rules
   */
  public async listFieldFilterRules(projectId: string, pipelineId: string): Promise<IFieldFilterRule[]> {
    const input: QueryCommandInput = {
      TableName: clickStreamTableName,
      KeyConditionExpression: '#id = :id AND begins_with(#type, :typePrefix)',
      FilterExpression: 'pipelineId = :pipelineId AND deleted = :d',
      ExpressionAttributeNames: {
        '#id': 'id',
        '#type': 'type',
      },
      ExpressionAttributeValues: {
        ':id': projectId,
        ':typePrefix': 'FILTER_RULE#',
        ':pipelineId': pipelineId,
        ':d': false,
      },
    };
    const records = await query(input);
    return records as IFieldFilterRule[];
  };
}