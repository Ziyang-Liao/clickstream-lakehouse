...
export interface S3TablesModelingConfig {
  ...
  readonly namespace?: string; // Namespace, default: clickstream_{projectId}
  readonly scheduleExpression: string; // Schedule expression
  readonly dataRetentionDays?: number; // Data retention days, default: 365
}
...
export interface DataModeling {
  ...
  readonly s3Tables?: S3TablesModelingConfig; // S3 Tables data modeling configuration
...
export class CPipeline {
  ...
  private _checkParametersAllowEdit(editedParameters: EditedPath[]): void {
    ...
    const AllowedList: string[] = [
      ...
      ...CS3TablesModelingStack.editAllowedList(),
  ...
  private async _getDataProcessingWorkflow(bucketName: string): Promise<WorkflowParallelBranch> {
    ...
    // Data modeling: Redshift and S3 Tables are mutually exclusive
    const dataModelingState = await this.getDataModelingState();
    const s3TablesModelingState = await this.getS3TablesModelingState();

    // Determine which modeling state to use (last step before reporting)
    if (dataModelingState && !s3TablesModelingState) {
      ...
      // Reporting follows Redshift modeling
      const reportingState = await this.getReportingState();
      if (reportingState) {
    ...
    } else if (s3TablesModelingState && !dataModelingState) {
      branch.States[PipelineStackType.DATA_MODELING_S3_TABLES] = s3TablesModelingState;
      if (athenaState) {
        branch.States[PipelineStackType.ATHENA].Next = PipelineStackType.DATA_MODELING_S3_TABLES;
        delete branch.States[PipelineStackType.ATHENA].End;
      } else {
        branch.States[PipelineStackType.DATA_PROCESSING].Next = PipelineStackType.DATA_MODELING_S3_TABLES;
        delete branch.States[PipelineStackType.DATA_PROCESSING].End;
      }
      // Reporting follows S3 Tables modeling
      const reportingState = await this.getReportingState();
      if (reportingState) {
        branch.States[PipelineStackType.REPORTING] = reportingState;
        branch.States[PipelineStackType.DATA_MODELING_S3_TABLES].Next = PipelineStackType.REPORTING;
        delete branch.States[PipelineStackType.DATA_MODELING_S3_TABLES].End;
      }
    } else if (dataModelingState && s3TablesModelingState) {
      // Both configured — this should not happen due to API validation.
      // Defensive: prefer Redshift (existing behavior) and log warning.
      logger.warn('Both Redshift and S3 Tables modeling configured — using Redshift only');
      branch.States[PipelineStackType.DATA_MODELING_REDSHIFT] = dataModelingState;
      if (athenaState) {
        branch.States[PipelineStackType.ATHENA].Next = PipelineStackType.DATA_MODELING_REDSHIFT;
        delete branch.States[PipelineStackType.ATHENA].End;
  ...
  private async getS3TablesModelingState(): Promise<WorkflowState | undefined> {
    ...
    const s3TablesModelingTemplateURL = await this.getTemplateUrl(PipelineStackType.DATA_MODELING_S3_TABLES);
    if (!s3TablesModelingTemplateURL) {
      throw new ClickStreamBadRequestError('Template: s3-tables-modeling not found in dictionary.');
    }

    const s3TablesModelingStack = new CS3TablesModelingStack(this.pipeline, this.resources!);
    const s3TablesModelingStackParameters = getStackParameters(
      s3TablesModelingStack, SolutionVersion.Of(this.pipeline.templateVersion ?? FULL_SOLUTION_VERSION));
    const s3TablesModelingStackName = getStackName(
      this.pipeline.pipelineId, PipelineStackType.DATA_MODELING_S3_TABLES, this.pipeline.ingestionServer.sinkType);
    const s3TablesModelingState: WorkflowState = {
      ...
      Data: {
        Input: {
          Action: 'Create',
          Region: this.pipeline.region,
          StackName: s3TablesModelingStackName,
          TemplateURL: s3TablesModelingTemplateURL,
          Parameters: s3TablesModelingStackParameters,
          Tags: this.stackTags,
        },
        Callback: {
          BucketName: stackWorkflowS3Bucket ?? '',
