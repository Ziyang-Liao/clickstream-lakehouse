...
export class CS3TablesModelingStack extends JSONObject {
  ...
  @JSONObject.optional('')
  @JSONObject.custom((stack: CS3TablesModelingStack, key: string, _value: any) => {
    const defaultValue = stack._pipeline?.network.vpcId;
    validatePattern(key, VPC_ID_PATTERN, defaultValue);
    return defaultValue;
  })
  ...
  @JSONObject.optional('')
  @JSONObject.custom((stack: CS3TablesModelingStack, _key: string, _value: any) => {
    return stack._pipeline?.projectId;
  })
  ProjectId?: string;
  ...
  @JSONObject.optional('')
  @JSONObject.custom((stack: CS3TablesModelingStack, _key: string, _value: any) => {
    return stack._pipeline?.dataModeling?.s3Tables?.tableBucketArn ?? '';
  })
  S3TableBucketArn?: string;

  @JSONObject.optional('')
  @JSONObject.custom((stack: CS3TablesModelingStack, _key: string, _value: any) => {
    const namespace = stack._pipeline?.dataModeling?.s3Tables?.namespace;
    if (namespace) {
      return namespace;
    }
    // Default namespace: clickstream_{projectId}
    return `clickstream_${stack._pipeline?.projectId ?? ''}`;
  })
  S3TableNamespace?: string;

  @JSONObject.optional('cron(0 2 * * ? *)')
  @JSONObject.custom((stack: CS3TablesModelingStack, key: string, _value: any) => {
    const defaultValue = stack._pipeline?.dataModeling?.s3Tables?.scheduleExpression ?? 'cron(0 2 * * ? *)';
    validatePattern(key, SCHEDULE_EXPRESSION_PATTERN, defaultValue);
    return defaultValue;
  })
  ScheduleExpression?: string;
  ...
  @JSONObject.lte(3650)
  @JSONObject.custom((stack: CS3TablesModelingStack, _key: string, _value: any) => {
    return stack._pipeline?.dataModeling?.s3Tables?.dataRetentionDays ?? 365;
  })
  ...
  @JSONObject.optional('')
  @JSONObject.custom((_stack: CS3TablesModelingStack, _key: string, _value: string) => {
    return getIamRoleBoundaryArn();
  })
  IamRoleBoundaryArn?: string;

  constructor(pipeline: IPipeline, resources: CPipelineResources) {
    if (!pipeline.dataModeling?.s3Tables) {
      throw new ClickStreamBadRequestError('S3 Tables configuration is required for S3 Tables data modeling.');
    }

    if (isEmpty(pipeline.dataModeling.s3Tables.tableBucketArn)) {
      throw new ClickStreamBadRequestError('S3 Table Bucket ARN is required for S3 Tables data modeling.');
    }

    if (isEmpty(pipeline.dataModeling.s3Tables.scheduleExpression)) {
      throw new ClickStreamBadRequestError('Schedule expression is required for S3 Tables data modeling.');
    }
