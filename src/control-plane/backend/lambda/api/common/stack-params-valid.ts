...
import { CPipelineResources, IPipeline, S3TablesModelingConfig } from '../model/pipeline';
...
// S3 Table Bucket ARN pattern: arn:aws:s3tables:<region>:<account-id>:bucket/<bucket-name>
export const S3_TABLE_BUCKET_ARN_PATTERN = '^arn:aws(-cn|-us-gov)?:s3tables:[a-z0-9-]+:[0-9]{12}:bucket/[a-z0-9][a-z0-9.-]{1,61}[a-z0-9]$';
...
/**
 * Validates that S3 Tables and Redshift data modeling configurations are mutually exclusive.
 * Property 1: S3 Tables and Redshift configuration mutual exclusion
 * Validates: Requirements 1.6, 8.5
 */
export const validateDataModelingMutualExclusion = (pipeline: IPipeline) => {
  const hasRedshift = !isEmpty(pipeline.dataModeling?.redshift);
  const hasS3Tables = !isEmpty(pipeline.dataModeling?.s3Tables);

  if (hasRedshift && hasS3Tables) {
    throw new ClickStreamBadRequestError(
      'Redshift and S3 Tables data modeling are mutually exclusive. Please choose only one data modeling option.',
    );
  }
  return true;
};
...
/**
 * Validates S3 Tables configuration parameters.
 * Property 11: API configuration validation
 * Validates: Requirements 8.4
 */
export const validateS3TablesConfig = (s3TablesConfig: S3TablesModelingConfig) => {
  // Validate tableBucketArn format
  if (!isEmpty(s3TablesConfig.tableBucketArn)) {
    const arnRegex = new RegExp(S3_TABLE_BUCKET_ARN_PATTERN);
    if (!arnRegex.test(s3TablesConfig.tableBucketArn)) {
      throw new ClickStreamBadRequestError(
        'Invalid S3 Table Bucket ARN format. Expected format: arn:aws:s3tables:<region>:<account-id>:bucket/<bucket-name>',
      );
    }
  } else {
    throw new ClickStreamBadRequestError(
      'S3 Table Bucket ARN is required for S3 Tables data modeling.',
    );
  }

  // Validate scheduleExpression format
  if (!isEmpty(s3TablesConfig.scheduleExpression)) {
    validateScheduleExpression(s3TablesConfig.scheduleExpression);
  } else {
    throw new ClickStreamBadRequestError(
      'Schedule expression is required for S3 Tables data modeling.',
    );
  }

  // Validate dataRetentionDays range (1-3650)
  if (s3TablesConfig.dataRetentionDays !== undefined) {
    if (s3TablesConfig.dataRetentionDays < 1 || s3TablesConfig.dataRetentionDays > 3650) {
      throw new ClickStreamBadRequestError(
        'Data retention days must be between 1 and 3650.',
      );
    }
  }
...
/**
 * Validates schedule expression format (rate or cron).
 */
export const validateScheduleExpression = (scheduleExpression: string) => {
  if (scheduleExpression.startsWith('rate(') && scheduleExpression.endsWith(')')) {
    // Validate rate expression format: rate(value unit)
    const rateContent = scheduleExpression.substring(5, scheduleExpression.length - 1);
    const parts = rateContent.split(' ');
    if (parts.length !== 2) {
      throw new ClickStreamBadRequestError(
        'Invalid schedule expression format. Rate expression should be: rate(value unit)',
      );
    }
    ...
    if (isNaN(value) || value <= 0) {
      throw new ClickStreamBadRequestError(
        'Invalid schedule expression format. Rate value must be a positive integer.',
      );
    }
    const validUnits = ['minute', 'minutes', 'hour', 'hours', 'day', 'days'];
    if (!validUnits.includes(unit)) {
      throw new ClickStreamBadRequestError(
        'Invalid schedule expression format. Rate unit must be one of: minute(s), hour(s), day(s)',
      );
    }
    return true;
  } else if (scheduleExpression.startsWith('cron(') && scheduleExpression.endsWith(')')) {
    // Validate cron expression format
    const cronContent = scheduleExpression.substring(5, scheduleExpression.length - 1);
    const cronParts = cronContent.split(' ');
    if (cronParts.length !== 6) {
      throw new ClickStreamBadRequestError(
        'Invalid schedule expression format. Cron expression should have 6 fields: cron(Minutes Hours Day-of-month Month Day-of-week Year)',
      );
    }
    return true;
  }
  ...
  throw new ClickStreamBadRequestError(
    'Invalid schedule expression format. Must start with rate( or cron(',
  );
};
