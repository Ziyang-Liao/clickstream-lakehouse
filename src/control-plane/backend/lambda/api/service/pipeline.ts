...
import { FieldFilterService } from './field-filter';
...
const fieldFilterService = new FieldFilterService();
...
export class PipelineServ {
  ...
  public async add(req: any, res: any, next: any) {
    try {
      ...
      // Validate S3 Tables and Redshift mutual exclusion
      validateDataModelingMutualExclusion(body);

      // Validate S3 Tables configuration if present
      if (!isEmpty(body.dataModeling?.s3Tables)) {
        validateS3TablesConfig(body.dataModeling!.s3Tables!);
      }
  ...
  public async delete(req: any, res: any, next: any) {
    try {
      ...
      // Cascade delete all filter rules associated with this pipeline
      await fieldFilterService.deleteByPipeline(pid, id, operator);
